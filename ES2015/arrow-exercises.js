...
var mapFilterAndReduce = arr => arr.map(val => val.firstName).filter(name => name.length < 5).reduce((acc, next) => {
	acc[next] = next.length;
	return acc;
}, {});
...
// using curly braces lets the arrow function now that it needs to return an object
